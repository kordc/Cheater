...
class CyganikChwilczynski(Player):
    ...
    def play_card(self, card, declaration=None):
    ...
    def putCard(self, declared_card):
        ...
        #LIE always if you can
        if decision is None and len(self.cards) > 1:
            decision = self.play_card(card, self.card_to_lie(declared_card))
    ...
    def getCheckFeedback(self, checked, iChecked, iDrewCards, revealedCard, noTakenCards, log=False):
        ...
        if checked and revealedCard is not None and iChecked and not iDrewCards:
            ...
            if my_cards_taken > 0:
                self.known_opponent_cards.append(self.my_cards_on_pile[-my_cards_taken])
