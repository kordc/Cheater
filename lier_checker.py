...
class LierChecker(Player):
    ...
    def putCard(self, declared_card):
        ...
        # Don't lie if I can play properly
        # Don't lie if I have only last card (rules)
        # Don't lie if I'm winning by at least 2 cards
        lie_prop = np.random.uniform(0, 1)

        # Find if he's often checking, if yes lie less
        # lie_prop *= 1 - (self.number_of_opponents_checks + 1) / (self.number_of_moves + 1)
        if lie_prop > 0.7 and decision is None and len(self.cards) > 1:
            decision = self.cards[0], self.card_to_lie(declared_card)
            self.my_cards_on_pile.append(self.cards[0])
